...
export const AdminManagement: React.FC = () => {
  ...
  const [selectedStatus, setSelectedStatus] =
    useState<AdminFilter>('DISETUJUI');
  ...
  const fetchRequests = async () => {
    ...
    try {
      ...
      setErrorMsg(
        err?.message || 'Gagal memuat data pencetakan.'
      );
    } finally {
      setLoading(false);
    }
  ...
  const updatePrintStatus = async (
    ...
    try {
      ...
      if (newStatus === 'SEDANG_DICETAK') {
        setMsg(`${request.id} mulai diproses untuk pencetakan.`);
        setSelectedStatus('SEDANG_DICETAK');
      } else {
        setMsg(
          `${request.id} telah selesai dicetak dan masuk ke Riwayat Fotokopi.`
        );
  ...
  const handleExportCSV = () => {
    ...
    link.href = url;
    link.download = `Laporan_Fotokopi_${new Date()
      .toISOString()
      .slice(0, 10)}.csv`;
    ...
    <div className="max-w-7xl mx-auto px-4 py-8 space-y-6">
      <div className="bg-slate-900 text-white p-6 sm:p-8 rounded-xl border border-slate-800 flex flex-col lg:flex-row items-start lg:items-center justify-between gap-4 shadow-sm">
        <div>
          ...
          <p className="text-xs text-slate-300 mt-1">
            Pantau pengajuan yang telah disetujui Kepala
            Sekolah, proses pencetakan, dan riwayat bahan
            yang pernah difotokopi.
          </p>
        </div>
      ...
      <div className="bg-white rounded-xl p-4 border border-slate-200 shadow-sm flex flex-col lg:flex-row justify-between gap-4">
        <div className="flex gap-1 bg-slate-100 p-1 rounded-lg overflow-x-auto">
          {[
            ...
            <button
              ...
              type="button"
              onClick={() =>
                setSelectedStatus(status as AdminFilter)
              }
        ...
        <div className="relative w-full lg:w-96">
          ...
          <input
            type="text"
            placeholder="Cari guru, email, kelas, judul, atau Tracking ID..."
            value={search}
      ...
      {selectedRequest && (
        <div className="fixed inset-0 z-50 bg-slate-950/70 backdrop-blur-sm flex items-center justify-center p-4 overflow-y-auto">
          <div className="bg-white rounded-xl max-w-xl w-full shadow-2xl border border-slate-200 my-8">
            <div className="p-6 border-b border-slate-100 flex items-start justify-between gap-4">
              <div>
                <span className="text-[10px] text-blue-600 font-bold uppercase">
                  Detail Proses Fotokopi
                </span>
              ...
              <div className="pt-4 border-t border-slate-100">
                ...
                {selectedRequest.status === 'SELESAI' && (
                  <div className="bg-green-50 border border-green-200 rounded-lg p-4 text-center">
                    ...
                    <div className="text-[11px] text-green-700 mt-1">
                      Pengajuan ini tersimpan sebagai
